...
func IPv6Conv(a, b, c, d, e, f, g, h, i, j, k, l, m, n, o, p byte) net.IP {
	...
	/*
		...
		for _, a := range addrs {
			if ipnet, ok := a.(*net.IPNet); ok && !ipnet.IP.IsLoopback() {
				if len(ipnet.IP.To16()) == net.IPv6len {
		...
		return
	*/
	addrs := net.ParseIP("2001:df6:b800:1130:a163:44:152:206a")
	copy(addr[:], addrs)
	return addr
}

// Given a host name convert it to a 4 byte IP address.
func destAddr(dest string) (destAddr [16]byte, err error) {
...
func (hop *TracerouteHop) AddressString() string {
	return fmt.Sprintf("%v", IPv6Conv(hop.Address[0], hop.Address[1], hop.Address[2], hop.Address[3],hop.Address[4], hop.Address[5], hop.Address[6], hop.Address[7],hop.Address[8], hop.Address[9], hop.Address[10], hop.Address[11],hop.Address[12], hop.Address[13], hop.Address[14], hop.Address[15]))
...
func Traceroute(dest string, options *TracerouteOptions, c ...chan TracerouteHop) (result TracerouteResult, err error) {
	...
	for {
		...
		syscall.Bind(recvSocket, &syscall.SockaddrInet6{Port: options.Port(), Addr: socketAddr})
		...
		syscall.Sendto(sendSocket, []byte{0x0}, 0, &syscall.SockaddrInet6{Port: options.Port(), Addr: destAddr})
		...
		if err == nil {
			...
			currAddr := from.(*syscall.SockaddrInet6).Addr
			fmt.Println(currAddr)
